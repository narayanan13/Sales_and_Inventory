				...
				<form action="" id="manage-sales">
					...
					<div class="col-md-12">
						<div class="row">
							...
							<div class="bill-date col-md-3" style="margin-left: 70px;">
								...
								{
									$val_id = $_GET['id'];
									$customer = $conn->query("SELECT date_created FROM sales_list where id = $val_id");
									if($row=$customer->fetch_assoc())
									{
						...
						<div class="row mb-3">
								<div class="col-md-4">
									...
									while($row=$product->fetch_assoc()):
										$prod[$row['id']] = $row;
									?>
										<option value="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-hsn_code="<?php echo $row['hsn_code'] ?>"><?php echo $row['name'] . ' | ' . $row['hsn_code'] ?></option>
									<?php endwhile; ?>
									</select>
								</div>
									...
									?>
										<tr class="item-row">
											<td>
												...
												<p class="pdesc"><small><i>HSN Code: <b><?php echo $prod[$row['product_id']]['hsn_code'] ?></b></i></small></p>
											</td>
											<td>
												<input type="hidden" min="1" step="any" name="qty[]" value="<?php echo $row['qty'] ?>" class="text-right">
											...
											<td>
												<input type="hidden" min="1" step="any" name="gst[]" value="<?php echo $prod[$row['product_id']]['gst'] ?>" class="text-right">
												<p class="text-right"><?php echo $prod[$row['product_id']]['gst'] ?></p>
											</td>
											<td>
												<p class="total text-right"></p>
											</td>
					      ...
					      <div class="modal-body">
					      	<div class="container-fluid">
					      		<div class="form-group">
					      			...
					      			<input type="text" name="netamount" value="" class="form-control text-right" readonly="">
					      		</div>
								  <div class="form-group">
					      			<label for="" class="control-label">GST Amount (₹)</label>
					      			<input type="text" name="gstamount" value="" class="form-control text-right" readonly="">
					      		</div>
								  <div class="form-group">
					      		...
					      		<!-- <div class="form-group">
					      			<label for="" class="control-label">Amount Given</label>
					      			<input type="number" name="amount_tendered" value="0" min="0" class="form-control text-right" >
					      		</div>
					      		<div class="form-group">
					      			<label for="" class="control-label">Balance Amount</label>
...
<div id="tr_clone">
	...
	<tr class="item-row">
		<td>
			...
			<p class="pdesc"><small><i>HSN Code: <b>Description</b></i></small></p>
		</td>
		<td>
		...
		<td>
			<input type="hidden" min="1" step="any" name="gst[]" value="" class="text-right" readonly="">
			<p class="gst text-right"></p>
		</td>
		<td>
			<p class="total text-right"></p>
		</td>
...
<script>
	...
	$('#pay').click(function(){
		// console.log($("#list .item-row").length)
	...
	$('#add_list').click(function(){
		...
		var product = $('#product').val(),
			...
			if(bill_date == ''){
				alert_toast("Please Choose Date",'danger')
				return false;
			}
			...
			$.ajax({
				...
				success:function(resp){
					...
					if(resp.available >= qty){
						...
						tr.find('.pdesc b').html($("#product option[value='"+product+"']").attr('data-hsn_code'))
						tr.find('.price').html(resp.price)
						tr.find('.gst').html(resp.gst)
						tr.find('[name="product_id[]"]').val(product)
						tr.find('[name="qty[]"]').val(qty)
						tr.find('[name="gst[]"]').val(resp.gst)
						...
						var total = parseFloat(resp.price) * parseFloat(qty) + (parseFloat(resp.price) * parseFloat(qty) * parseFloat(resp.gst)/100);

						tr.find('.total').html(parseFloat(total).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))
	...
	function calculate_total(){
		...
		var n_amnt = parseFloat(_this.find('[name="qty[]"]').val()) * parseFloat(_this.find('[name="price[]"]').val());
		var n_gst_amt = parseFloat(_this.find('[name="qty[]"]').val()) * parseFloat(_this.find('[name="price[]"]').val()) * parseFloat(_this.find('[name="gst[]"]').val())/100;
		var amount = parseFloat(n_amnt) + parseFloat(n_gst_amt);
		amount = amount > 0 ? amount :0;
		_this.find('p.amount').html(parseFloat(n_amnt).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))
		_this.find('p.total').html(parseFloat(amount).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))
		amnt+=parseFloat(n_amnt);
		gst_amt+=parseFloat(n_gst_amt);
		total+=parseFloat(amount);
		})
		// console.log(amnt + "/" + gst_amt + '/' + total)
		...
		$('[name="netamount"]').val(amnt)
		$('#list .netamount').html(parseFloat(amnt).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))
		$('[name="gstamount"]').val(gst_amt)
		$('#list .gstamount').html(parseFloat(gst_amt).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))

	}
	// $('[name="amount_tendered"]').keyup(function(){
	// 	var tendered = $(this).val();
	// 	var tamount = $('[name="tamount"]').val();
	// 	var gstamount = $('[name="gstamount"]').val();
	// 	var netamount = $('[name="netamount"]').val();
