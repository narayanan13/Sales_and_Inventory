					...
					<div class="col-md-12">
						<div class="row">
							...
							<div class="bill-date col-md-3" style="margin-left: 70px;">
								...
								{
									$val_id = $_GET['id'];
									$customer = $conn->query("SELECT date_created FROM receiving_list where id = $val_id");
...
<div id="tr_clone">
	...
	<tr class="item-row">
		...
		<td>
			<input type="number" min="1" step="any" name="gst[]" value="" class="text-right" readonly="">
...
<script>
		...
		var product = $('#product').val(),
			...
			$.ajax({
				...
				success:function(resp){
					...
					var total = parseFloat(price) * parseFloat(qty) + (parseFloat(price) * parseFloat(qty) * parseFloat(resp.gst)/100);
	...
	function calculate_total(){
		...
		var amnt = parseFloat(_this.find('[name="qty[]"]').val()) * parseFloat(_this.find('[name="price[]"]').val());
		var amount = parseFloat(_this.find('[name="qty[]"]').val()) * parseFloat(_this.find('[name="price[]"]').val()) +(+ (parseFloat(_this.find('[name="qty[]"]').val()) * parseFloat(_this.find('[name="price[]"]').val()) * parseFloat(_this.find('[name="gst[]"]').val())/100));
		amount = amount > 0 ? amount :0;
		_this.find('p.amount').html(parseFloat(amnt).toLocaleString('en-US',{style:'decimal',maximumFractionDigits:2,minimumFractionDigits:2}))
